...
class Rev(object):
    def __init__(self, master):
        ...
        up = Button(self.container, text="Up a folder", command=self.go_up)
        new_folder = Button(self.container, text="New Folder", command=self.new_f)
        new_window = Button(self.container, text="New Window", command=self.new_windo)
        new_file = Button(self.container, text="New File", command=self.new_fi)
        deleter = Button(self.container, text="Delete File", command=self.delete_file)
        empty = Button(self.container, text="Empty Trash", command=self.empt)
        recover = Button(self.container, text="Get back from Trash", command=self.recover_trash)
        move_things = Button(self.container, text="Move something", command=self.move_anywhere)
        copy_things = Button(self.container, text="Copy something", command=self.copy_anywhere)
        open_in_term = Button(self.container, text="Open dir in Terminal", command=self.open_in_terminal)
        duplicate = Button(self.container, text="Duplicate", command=self.dup)
        rename = Button(self.container, text="Rename", command=self.rename_file)
        system("mkdir ~/Trash/")
        self.searcher.pack()
        confirm.pack()
        self.listbox_thing.pack()
        self.listbox_thing.bind("<Control-b>", self.focus_buttons)
        up.pack(side=LEFT)
    ...
    def focus_buttons(self, value=None):
    ...
    def rename_to(self, value=None):
        self.full = str(path.dirname(path.realpath(self.listbox_thing.get(ACTIVE)))) + "/" + str(self.listbox_thing.get(ACTIVE))
        i = ""
        for x in range(0, len(self.full)):
            if self.full[x] == " ":
                i += "\\"
            i += self.full[x]
    ...
    def dup(self, value=None):
        ...
        for x in range(0, len(self.full)):
            if self.full[x] == " ":
                i += "\\"
            i += self.full[x]
        o = subprocess.Popen("cp " + i + " " + i + str(self.v) + "; ls", \
            shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        y, x = o.communicate()
        if x:
            self.warn(x)
    ...
    def open_in_terminal(self, value=None):
        m = subprocess.Popen("ls", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        k, y = m.communicate()
        system("open -a Terminal.app " + str(path.dirname(path.realpath(k))))
    ...
    def new_fil(self, value=None):
        o = subprocess.Popen("touch " + str(self.new_f_name.get()), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        y, x = o.communicate()
        if x:
            self.warn(x)
        else:
    ...
    def new_fol(self, value=None):
        o = subprocess.Popen("mkdir " + str(self.new_name.get()), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        y, x = o.communicate()
        if x:
            self.warn(x)
        else:
    ...
    def search(self, value=None):
        chdir("/")
        p = subprocess.Popen("find ~ -name " + self.searcher.get(), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def open_it(self, value=None):
        self.full = str(path.dirname(path.realpath(self.listbox_thing.get(ACTIVE)))) + "/" + self.listbox_thing.get(ACTIVE)
        if self.full[-4:] == ".app":
            i = ""
            for x in range(0, len(self.full)):
                if self.full[x] == " ":
